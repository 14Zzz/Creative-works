...
Graph<int> makeGraph()
{
	...
	int amountEdges, sourceVertex, targetVertex, edgeWeight; // ñîçäàíèå íåîáõîäèìûõ äëÿ ââîäà ãðàôà ïåðåìåííûõ
...
void mouseClick(int btn, int stat, int x, int y) {
	...
	if (stat == GLUT_DOWN) {
		...
		if (x > shift&& x < shift + 135 && y >  shift  && y <  shift+30)
		{
			...
			cout << "Èñõîäíàÿ âåðøèíà: "; cin >> sourceVertex; cout << endl; 
		...
		if (x > shift&& x < shift + 135 && y > shift +40 && y <  shift +70)
		{
			...
			cout << "Ââåäèòå âåðøèíó: "; cin >> sourceVertex; cout << endl;
			int* sourceVertPtr = &sourceVertex;
